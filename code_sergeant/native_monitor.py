...
class NativeMonitor:
    ...
    def detect_activity_change(self, current_title: str) -> bool:
        ...
        """
        changed = (
            self.last_window_title is not None
            and self.last_window_title != current_title
        )
        if changed:
    ...
    def get_current_activity(self) -> ActivityEvent:
        ...
        try:
            ...
            is_thinking = False
            if (
                self.THINKING_IDLE_MIN <= idle_seconds <= self.THINKING_IDLE_MAX
                and self.is_productive_app(app)
            ):
                is_thinking = True
                logger.debug(
                    f"Detected thinking state: idle {idle_seconds:.0f}s in {app}"
                )
