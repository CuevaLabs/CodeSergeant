...
class MotivationMonitor:
    ...
    def __init__(
        ...
        logger.info(f"MotivationMonitor initialized (check every {check_interval_minutes} min)")
    ...
    def _check_motivation(self):
        ...
        if old_state is None or old_state.state != new_state.state:
            logger.info(f"Motivation state changed: {old_state.state if old_state else 'None'} -> {new_state.state}")
    ...
    def _detect_state_rules(
        ...
        if (focus_minutes >= self.FLOW_MIN_FOCUS_MINUTES and
            app_switches <= self.FLOW_MAX_APP_SWITCHES and
            idle_seconds <= self.FLOW_MAX_IDLE_SECONDS):
        ...
        if self.personality_manager:
            if state.state == "struggling":
                ...
                if state.confidence > 0.7:
                    phrase = self.personality_manager.get_phrase("encouragement_general")
    ...
    def handle_user_statement(self, statement: str) -> Optional[str]:
        ...
        stuck_keywords = ["stuck", "can't figure", "don't know", "confused", "lost", "no idea"]
        focus_keywords = ["can't focus", "distracted", "keep thinking", "mind wandering"]
        ...
        if any(kw in statement_lower for kw in stuck_keywords):
            if self.personality_manager:
                ...
                response = "Let's break this down. What's the smallest next step you can take?"
            ...
            if self.personality_manager:
                ...
                response = "Remember why you started. One small win can rebuild momentum."
