...
from .models import ActivityEvent, Judgment, SessionStats, PomodoroState
from .native_monitor import NativeMonitor
from .judge import ActivityJudge
from .tts import TTSService
from .reminders import ReminderWorker
from .storage import write_session_log, add_voice_note, add_distraction_log, add_annotation, save_note_to_file
from .config import load_config, save_config, get_personality_name, update_personality
...
class AppController:
    ...
    def __init__(self):
        ...
        self.tts_service = TTSService(
            ...
            api_key=os.getenv("ELEVENLABS_API_KEY") or self.config["tts"].get("elevenlabs_api_key") or self.config["tts"].get("api_key"),
        ...
        if self.wake_word_detector and self.config.get("voice_activation", {}).get("enabled", False):
        ...
        self.drill_interval = 1.0  # Drill every 1 second when off_task for fast response
        ...
        self.motivation_monitor = MotivationMonitor(
            ...
            check_interval_minutes=self.config.get("motivation", {}).get("check_interval_minutes", 3)
        )
    ...
    def _init_wake_word_detector(self):
        ...
        if self.config.get("voice_activation", {}).get("enabled", False):
            ...
            self.wake_word_detector = WakeWordDetector(
                ...
                sensitivity=self.config.get("voice_activation", {}).get("sensitivity", 0.5),
    ...
    def end_session(self) -> None:
        ...
        if self.state.stats.start_time:
            ...
            self.state.stats.pomodoros_completed = self.pomodoro.state.pomodoros_completed
    ...
    def _start_drill_worker(self) -> None:
        ...
        def drill_loop():
            ...
            while not self.drill_stop_event.is_set() and self.state.session_active:
                ...
                if self.state.last_judgment_obj and self.state.last_judgment_obj.classification == "off_task":
    ...
    def _trigger_immediate_judgment(self) -> None:
        ...
        try:
            ...
            if judgment:
                ...
                self.state.last_judgment = f"{judgment.classification} ({judgment.confidence:.0%})"
                self.state.last_judgment_obj = judgment
                
                logger.info(f"Immediate judgment: {judgment.classification} ({judgment.confidence:.0%})")
    ...
    def change_goal(self, new_goal: str) -> None:
        ...
        if self.state.stats:
            add_annotation(self.state.stats, f"Goal changed from '{old_goal}' to '{new_goal}'")
    ...
    def set_personality(self, personality_name: str, custom_description: str = None,
                       custom_wake_word: str = None) -> None:
        """
        ...
        self.config = update_personality(
            self.config, personality_name,
            custom_description, custom_wake_word
        )
        ...
        self.tts_service.speak(f"Personality changed. Say '{self.state.wake_word}' to talk to me!")
    ...
    def start_note_taking(self) -> None:
        ...
        def note_taking_thread():
            try:
                ...
                if cleared > 0:
                    logger.info(f"Cleared {cleared} pending TTS messages before note-taking")
                ...
                logger.info(f"Note-taking: Recording until stop phrase (max {max_note_duration}s)...")
    ...
    def _start_workers(self):
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if last_activity is None or (
                        activity.app != last_activity.app or
                        activity.title != last_activity.title
                    ):
                        self.event_queue.put({
            ...
            while not self.stop_event.is_set():
                try:
                    ...
                    if self.current_activity:
                        ...
                        if (
                            last_judged_activity_id != current_activity_id or
                            last_judged_activity_id is None
                        ):
                        ...
                        if current_judgment:
                            if current_judgment.action in ("warn", "yell"):
                                ...
                                # Update focus streak
                                self.state.stats.current_focus_streak_seconds += judge_interval
                                if self.state.stats.current_focus_streak_seconds > self.state.stats.best_focus_streak_seconds:
                                    self.state.stats.best_focus_streak_seconds = self.state.stats.current_focus_streak_seconds
        ...
        self.workers["activity_poller"] = threading.Thread(target=activity_poller_loop, daemon=True)
        self.workers["judge_worker"] = threading.Thread(target=judge_worker_loop, daemon=True)
        self.workers["reminder_worker"] = threading.Thread(target=reminder_worker.run, daemon=True)
