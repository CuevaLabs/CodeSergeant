...
class ActivityJudge:
        ...
        try:
            ...
            if judgment.action == "yell" and last_yell_time:
                ...
                if time_since_yell < cooldown_seconds:
                    logger.debug(
                        f"Suppressing yell due to cooldown ({time_since_yell:.1f}s < {cooldown_seconds}s)"
                    )
    ...
    def _judge_with_llm(
        self, goal: str, activity: ActivityEvent, history: list[ActivityEvent]
    ) -> Judgment:
        """
        ...
        if self.ai_client:
            try:
                ...
                logger.debug(
                    f"LLM judgment: {judgment.classification} ({judgment.confidence:.0%})"
                )
                return judgment
...
- Keyboard active: {activity.keyboard_active}"""
        ...
        if self.detect_goal_drift():
            drift_warning = (
                "\n\nWARNING: User appears to be gradually drifting from "
                "their goal. Consider a firmer response."
            )
...
JSON only:"""
    ...
    def _validate_judgment(
        self, judgment_dict: dict, activity_context: str = ""
    ) -> Judgment:
        ...
        if classification not in valid_classifications:
            logger.warning(
                f"Invalid classification: {classification}, defaulting to unknown"
            )
            ...
            for keyword in self.DISTRACTION_KEYWORDS:
                if keyword in activity_lower:
                    if classification != "off_task":
                        logger.info(
                            f"Override: Forcing off_task for '{keyword}' in activity"
                        )
    ...
    def _judge_fallback(self, goal: str, activity: ActivityEvent) -> Judgment:
        ...
        if is_productive:
            ...
            if activity.is_thinking or (
                activity.idle_duration_seconds >= 30
                and activity.idle_duration_seconds <= 180
            ):
                return Judgment(
                    classification="thinking",
                    confidence=0.7,
