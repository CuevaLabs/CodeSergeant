...
from .models import DistractionLog, SessionStats, VoiceNote
...
def add_voice_note(
    stats: SessionStats, content: str, transcription: str = ""
) -> VoiceNote:
    """
    ...
    """
    note = VoiceNote(
        timestamp=datetime.now(), content=content, transcription=transcription
    )
...
def add_distraction_log(
    stats: SessionStats, reason: str, is_phone: bool = False
) -> DistractionLog:
    """
    ...
    log_entry = DistractionLog(
        timestamp=datetime.now(), reason=reason, is_phone=is_phone
    )
...
def get_session_summary(stats: SessionStats) -> Dict[str, Any]:
    ...
    total_seconds = (
        stats.focus_seconds
        + stats.idle_seconds
        + stats.off_task_seconds
        + stats.thinking_seconds
    )

    return {
        "total_minutes": total_seconds // 60,
        "focus_minutes": stats.focus_seconds // 60,
        "focus_percentage": (stats.focus_seconds / total_seconds * 100)
        if total_seconds > 0
        else 0,
...
def export_session_to_markdown(
    stats: SessionStats, goal: str, output_file: str = None
) -> str:
    """
