...
from .dashboard import create_dashboard, DashboardWindow
...
class CodeSergeantApp(rumps.App):
    ...
    def __init__(self):
        ...
        self.dashboard: Optional[DashboardWindow] = create_dashboard(
            on_start=self._dashboard_start_session,
            on_end=self._dashboard_end_session
        )
        
        # Timer to poll controller state and process events
        self.timer = rumps.Timer(self._update_state, 0.5)  # Update every 0.5s for smooth updates
        self.timer.start()
    ...
    def _setup_menu(self):
        ...
        self.talk_item = rumps.MenuItem(f"Talk to {personality_name}", callback=self._on_talk_to_assistant)
        ...
        self.menu = [
            ...
            (
                ...
                [
                    ...
                    rumps.MenuItem("Start Break", callback=self._on_pomodoro_start_break),
            ...
            (
                ...
                [
                    (
                        ...
                        [
                            rumps.MenuItem("🎖️ Drill Sergeant", callback=lambda _: self._set_personality("sergeant")),
                            rumps.MenuItem("👋 Friendly Buddy", callback=lambda _: self._set_personality("buddy")),
                            rumps.MenuItem("📋 Professional Advisor", callback=lambda _: self._set_personality("advisor")),
                            rumps.MenuItem("🏆 Motivational Coach", callback=lambda _: self._set_personality("coach")),
                            rumps.MenuItem("✨ Custom...", callback=self._on_custom_personality),
                        ]
                    ),
                    rumps.MenuItem("Voice Settings...", callback=self._on_voice_settings),
                    rumps.MenuItem("AI Settings...", callback=self._on_ai_settings),
                    rumps.MenuItem("Pomodoro Settings...", callback=self._on_pomodoro_settings),
                    rumps.MenuItem("Toggle Wake Word", callback=self._on_toggle_wake_word),
                    None,  # Separator
                    rumps.MenuItem("Toggle Screen Monitoring", callback=self._on_screen_monitoring_toggle),
                ]
            ),
            None,  # Separator
    ...
    def _update_title(self, state):
        ...
        if state.session_active and state.current_activity:
            ...
            max_len = 15 if state.pomodoro_state and state.pomodoro_state.current_state != "stopped" else 25
    ...
    def _dashboard_start_session(self, goal: str, work_minutes: int, break_minutes: int):
    ...
    def _set_personality(self, personality_name: str):
        ...
        if hasattr(self, 'talk_item') and self.talk_item:
            self.talk_item.title = f"Talk to {self.controller.state.personality_name.title()}"
        if hasattr(self, 'wake_word_item') and self.wake_word_item:
            wake_word = self.controller.state.wake_word
        ...
        if response.clicked and response.text.strip():
            ...
            if success:
                ...
                rumps.notification(
                    ...
                    message=("OpenAI enabled!" if using_openai else "Key saved to .env. Install `openai` and restart.")
                )
            else:
                rumps.alert(
                    title="Error",
    ...
    def _on_pomodoro_settings(self, _):
        ...
        if response.clicked and response.text.strip():
            try:
                ...
                if 1 <= new_work <= 120:
                    self.controller.config["pomodoro"]["work_duration_minutes"] = new_work
    ...
    def _on_toggle_wake_word(self, _):
        ...
        current = self.controller.config.get("voice_activation", {}).get("enabled", False)
        new_state = not current
