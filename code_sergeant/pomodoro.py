...
class PomodoroTimer:
    ...
    def __init__(
        ...
        logger.info(
            f"PomodoroTimer initialized: work={work_duration_minutes}m, "
    ...
    def _complete_work(self):
        ...
        with self._lock:
            ...
            if (
                self.state.pomodoros_completed % self.state.pomodoros_until_long_break
                == 0
            ):
                self.state.current_state = "long_break"
                self.state.time_remaining_seconds = self.state.long_break_minutes * 60
                logger.info(
                    f"Work complete! Starting long break. Pomodoros: {self.state.pomodoros_completed}"
                )
            else:
                self.state.current_state = "short_break"
                self.state.time_remaining_seconds = self.state.short_break_minutes * 60
                logger.info(
                    f"Work complete! Starting short break. Pomodoros: {self.state.pomodoros_completed}"
                )
