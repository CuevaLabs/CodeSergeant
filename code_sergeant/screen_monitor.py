...
try:
    ...
    PIL_AVAILABLE = False
    logger.warning("Pillow not installed - blur regions unavailable. Install with: pip install Pillow")
...
class ScreenMonitor:
    ...
    DEFAULT_BLOCKLIST = [
        ...
        "Capital One", "US Bank", "PNC", "TD Bank", "USAA",
    ...
    def __init__(
        ...
        logger.info(f"ScreenMonitor initialized (enabled={self.enabled}, local={self.use_local_vision}, backend={self._vision_backend_status})")
    ...
    def _check_and_configure_vision_backend(self):
        ...
        if self.use_local_vision:
            ...
            if is_available:
                ...
                logger.info("✅ Screen monitoring: Using local LLaVA via Ollama (privacy-first)")
            else:
                ...
                if self.ai_client.is_openai_available():
                    ...
                    logger.info("⚡ Screen monitoring: Auto-switched to OpenAI GPT-4V (Ollama unavailable)")
                    logger.info("💡 Tip: Install Ollama from https://ollama.com/download for local-only privacy")
                else:
                    # No vision backend available - disable screen monitoring
                    self.enabled = False
                    self._vision_backend_status = "disabled"
                    logger.error("❌ Screen monitoring disabled: No vision backend available")
                    logger.error("   → Either install Ollama: https://ollama.com/download")
            ...
            if self.ai_client.is_openai_available():
                ...
                if is_available:
                    ...
                    logger.warning("⚠️ OpenAI not available, using local Ollama instead")
                else:
                    self.enabled = False
                    self._vision_backend_status = "disabled"
                    logger.error("❌ Screen monitoring disabled: No vision backend available")
    ...
    def remove_blur_region(self, index: int):
        ...
        if 0 <= index < len(self.blur_regions):
            ...
            logger.info(f"Removed blur region: {removed.name or f'{removed.x},{removed.y}'}")
...
"""
        ...
        try:
            ...
            response = self.ai_client.analyze_image(
                screenshot_bytes,
                prompt,
                use_local=self.use_local_vision
            )
            ...
            if "ollama" in error_msg.lower() or "localhost:11434" in error_msg.lower() or "llava" in error_msg.lower():
                ...
                logger.info("   → Or set use_local_vision: false in config.json to use OpenAI")
                ...
                logger.info("💡 Tip: Either install Ollama or set up an OpenAI API key for screen monitoring")
    ...
    def _parse_analysis(self, response: str) -> ScreenAnalysis:
        ...
        if any(word in response_lower for word in ["coding", "writing", "editing", "working", "creating", "developing"]):
            progress = "making_progress"
            confidence = 0.7
        elif any(word in response_lower for word in ["idle", "blank", "nothing", "no activity"]):
            progress = "idle"
            confidence = 0.6
        elif any(word in response_lower for word in ["stuck", "same", "unchanged", "no progress"]):
            progress = "stuck"
            confidence = 0.6
        else:
            progress = "unknown"
    ...
    def _perform_check(self):
        ...
        logger.info(f"Screen analysis: {analysis.progress_indicator} ({analysis.confidence:.0%})")
